...
export class MarkdownService {
  ...
  constructor(private sanitizer: DomSanitizer) {
    this.markdownIt = new MarkdownIt('commonmark').use(insertPlugin);
  }
...
export default function insertPlugin(md: MarkdownIt, options)
{
  const defaults = {
    disabled: true,
    divWrap: false,
  ...
  function todoify(token, lastId, options, TokenConstructor) {
    ...
    // checkbox
    token.children.unshift(endCheckbox(TokenConstructor))
  ...
  function makeCheckbox(token, id, options, TokenConstructor) {
    const checkbox = new TokenConstructor('checkbox_input', 'ion-checkbox', 1);
  ...
  function beginLabel(id, TokenConstructor) {
    const label = new TokenConstructor('label_open', 'ion-label', 1);
    label.attrs = [["for", id]];
    return label;
  }

  function endLabel(TokenConstructor) {
    return new TokenConstructor("label_close", "ion-label", -1);
  }

  function endCheckbox(TokenConstructor) {
    return new TokenConstructor("checkbox_close", "ion-checkbox", -1);
  }
...
// true block-level token with .tag=='label'
  ...
  md.core.ruler.after('inline', 'github-task-lists', tokenize);
}
